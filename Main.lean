...
structure ForEachModuleState where
  ...
  mods : Std.HashMap Name (System.FilePath × ModuleData) := default
  count := 0
...
@[inline] nonrec def ForEachModuleM.run (env : Kernel.Environment) (mods : (Std.HashMap Name (System.FilePath × ModuleData))) (x : ForEachModuleM α) (s : ForEachModuleState := {env, mods}) : IO (α × ForEachModuleState) :=
  x.run s

partial def getLeafModules (imports : Array Import) (mods? : Option (Std.HashMap Name (System.FilePath × ModuleData))) : ForEachModuleM $ Array (Name × ModuleData) := do
  let mut leafs := #[]
  for i in imports do
    if i.runtimeOnly || (← get).moduleNameSet.contains i.module then
      continue
    let mod ← if let some mods := mods? then
        let some (_, mod) := mods.get? i.module | throw <| IO.userError s!"could not find module {i.module} in provided module data"
        pure mod
      else
        ...
        pure mod
    let modLeafs ← getLeafModules mod.imports mods?
...
partial def forEachModule' (f : Name → ModuleData → NameSet → ForEachModuleM NameSet) (imports : Array Import) (aborted : NameSet) (mods? : Option (Std.HashMap Name (System.FilePath × ModuleData)) := none) : ForEachModuleM NameSet := do
  let mut aborted := aborted
  while true do
    let leafs ← getLeafModules imports mods?
  ...
  match mod with
    ...
    | m =>
      ...
      if let some onlyConsts := onlyConsts? then
        ...
        let mut mods : Std.HashMap Name (System.FilePath × ModuleData) := default

        let appendMod imports env n aborted mods := do
          ...
          pure $ mods.insert n (modPath, mod)
        ...
        mods ← appendMod #[] env patchPreludeModName aborted mods

        let (aborted, s) ← ForEachModuleM.run env mods do
        ...
        --     if let some mfile ← SearchPath.findWithExt [cachedPath] "olean" n then
        ...
        let (_, s) ← ForEachModuleM.run env mods do
          forEachModule' (imports := #[m]) (aborted := aborted) fun dn d aborted => do
            -- unless not (← skip dn) do return aborted
            ...
            IO.println s!">>{dn} module [{(← get).count}/{numMods}] ({(← get).env.constants.map₂.toList.length} constants)"
            ...
            IO.println s!">>Finished {dn} module ({env.constants.map₂.toList.length} constants)"
            let imports := if dn == `Init.Prelude then
                #[{module := `Init.PatchPrelude}] ++ d.imports
              else
                d.imports

            let mods ← appendMod imports env dn aborted (← get).mods
            modify fun s => {s with env, mods}
            pure aborted
        ...
        let (_, s) ← ForEachModuleM.run (← mkEmptyEnvironment).toKernelEnv default do
          forEachModule' (imports := #[m]) (aborted := aborted) (mods? := .some s.mods) fun dn d aborted => do
            -- unless not (← skip dn) do return aborted

            IO.println s!">>{dn} module [{(← get).count}/{numMods}] ({(← get).env.constants.map₂.toList.length} constants)"
            let env := (← get).env.toMap₁.withConsts fun c => {c with stage₁ := false}
            let env ← d.constants |>.foldlM (init := env) fun accEnv ci => do
              pure $ accEnv.addLevelTrans ci
            IO.println s!">>Finished {dn} module ({env.constants.map₂.toList.length} constants)"

            let mods ← appendMod d.imports env dn aborted (← get).mods
            modify fun s => {s with env, mods}
            pure aborted

        for (n, (modPath, mod)) in s.mods do
          saveModuleData modPath n mod
           
        replayFromEnv addDecl m s.env.toMap₁ (op := "typecheck") (opts := {proofIrrelevance := not opts.proofIrrelevance, kLikeReduction := not opts.kLikeReduction, univs := not opts.univs}) (initConsts := #[`String, `Lean.Name, `Lean.Name.str, `Lean.Name.anonymous])
