...
def checkInductiveTypes
    (nparams : Nat) (indTypes : Array InductiveType) (typesOnly : Bool) (mainTypes : NameSet)
  ...
  let rec loopInd dIdx (stats : InductiveStats) : M α := do
    if _h : dIdx < indTypes.size then
      ...
      if typesOnly || not (mainTypes.contains indType.name) then
          ...
          if i < nparams then
            if stats.indConsts.isEmpty then
              LwithLocalDecl name dom.consumeTypeAnnotations bi fun param => do
                ...
                loop stats (← whnf type (← lparams)) (i + 1) nindices fuel k
            else
              let param := stats.params[i]!
              let defEq ← isDefEqPure dom.toPExpr (← getType param.toPExpr) (← lparams)
              unless defEq do
            ...
            LwithLocalDecl name dom.consumeTypeAnnotations bi fun arg => do
              ...
              loop stats (← whnf type (← lparams)) i (nindices + 1) fuel k
        else
          if i != nparams then
            throw <| .other "number of parameters mismatch in inductive datatype declaration"
          k (← whnf type (← lparams)) stats nindices
      loop stats (← whnf type (← lparams)) 0 0 1000 fun type stats nindices => do
        let type ← ensureSortPure type (← lparams)
        let mut stats := stats
...
def declareInductiveTypes
    ...
    (isUnsafe : Bool) (numNested : Nat) (typesOnly : Bool) (mainTypes : NameSet) (env : Kernel.Environment) : Kernel.Environment :=
  ...
  let infos := if typesOnly then infos else (infos.filter (fun i => not (mainTypes.contains i.name)))
  infos.foldl add env
      ...
      | fuel+1 => do
        if let .forallE name dom body bi := t then
          if let some param := stats.params[i]? then
            unless ← isDefEqPure dom.toPExpr (← getType param.toPExpr) (← lparams) do
              throw <| .other
            ...
            let s ← ensureTypePure dom.toPExpr (← lparams)
      ...
      if let .forallE name dom body bi := type then
        LwithLocalDecl name dom.consumeTypeAnnotations bi fun arg => do
          ...
          if i ≥ stats.params.size then
            if !(← ensureTypePure dom.toPExpr (← lparams)).toExpr.sortLevel!.isZero then
...
def loopArgs1 (stats : InductiveStats) (type : Expr) (i : Nat) (indices : Array Expr)
  ...
  | fuel+1 => do
    if let .forallE name dom body bi := type then
      if i < stats.params.size then
        loopArgs1 stats (← whnf (body.instantiate1 stats.params[i]!).toPExpr (← lparams)) (i + 1) indices fuel k
      else
        LwithLocalDecl name dom.consumeTypeAnnotations bi fun arg => do
        loopArgs1 stats (← whnf (body.instantiate1 arg).toPExpr (← lparams)) i (indices.push arg) fuel k
    else
      k indices
...
def loopInd1 (dIdx : Nat) (recInfos : Array RecInfo) (k : Array RecInfo → M α) : M α := do
  if _h : dIdx < indTypes.size then
    loopArgs1 stats (← whnf indTypes[dIdx].type.toPExpr (← lparams)) 0 #[] 1000 fun indices =>
...
where
  ...
  | fuel+1 => do
    if let .forallE name dom body bi := uiTy.toExpr then
      LwithLocalDecl name dom.consumeTypeAnnotations bi fun arg => do
      loop (← whnf (body.instantiate1 arg).toPExpr (← lparams)) (xs.push arg) fuel
    else
      k uiTy xs
...
def run (lparams : List Name) (nparams : Nat) (types : List InductiveType)
    (numNested : Nat) (typesOnly : Bool) (mainTypes : NameSet) : M Kernel.Environment := do
  let isUnsafe := (← read).safety != .safe
  let indTypes := types.toArray
  Kernel.Environment.checkDuplicatedUnivParams lparams
  checkInductiveTypes nparams indTypes typesOnly mainTypes fun stats => do
  let withDeclTypes? m := do
    withEnv (declareInductiveTypes add stats lparams nparams indTypes isUnsafe numNested typesOnly mainTypes) do
      if typesOnly then
        pure $ ← TypeChecker.getKEnv
      else m
  withDeclTypes? do
...
-/
def mkAuxRecNameMap (types : List InductiveType) (newNames : List Name) :
...
def Kernel.Environment.addInductive (env : Kernel.Environment) (lparams : List Name) (nparams : Nat)
    (types : List InductiveType) (typesOnly := false) (isUnsafe allowPrimitive : Bool) :
  ...
  let mainTypes := .ofList allIndNames
  let newTypes := (res.types.map (·.name)).drop types.length
  let env' ← AddInductive.run add lparams nparams res.types numNested typesOnly mainTypes
    { env, allowPrimitive, safety := if isUnsafe then .unsafe else .safe, const := `inductive, lparams}
  if numNested == 0 then return env'
  let (recNames', recNameMap') := mkAuxRecNameMap types newTypes
  ...
  let processRec recName := do
    ...
    modify (fun e => add e <| .recInfo { recInfo with
